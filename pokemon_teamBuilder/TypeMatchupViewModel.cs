...
public class TypeMatchupViewModel : INotifyPropertyChanged
{
    ...
    private string _selectedAttackType;
    ...
    private string _selectedDefenseType1;
    public string SelectedDefenseType1
    {
        get => _selectedDefenseType1;
        set
        {
            _selectedDefenseType1 = value;
            OnPropertyChanged(nameof(SelectedDefenseType1));
            UpdateResult();
        }
    }

    private string _selectedDefenseType2;
    public string SelectedDefenseType2
    {
        get => _selectedDefenseType2;
        set
        {
            _selectedDefenseType2 = value;
            OnPropertyChanged(nameof(SelectedDefenseType2));
            UpdateResult();
        }
    }
    ...
    // type chart dictionary
    private readonly Dictionary<(string atk, string def), double> chart =
    new()
    {
        // Normal
    ...
    private void UpdateResult()
    {
        if (string.IsNullOrEmpty(SelectedAttackType) ||
            string.IsNullOrEmpty(SelectedDefenseType1))
        {
            Result = "";
            return;
        }
        ...
        // Apply first defensive type
        if (chart.TryGetValue((SelectedAttackType, SelectedDefenseType1), out double m1))
            multiplier *= m1;

        // Apply second defensive type if selected
        if (!string.IsNullOrEmpty(SelectedDefenseType2))
        {
            if (chart.TryGetValue((SelectedAttackType, SelectedDefenseType2), out double m2))
                multiplier *= m2;
        }

        // applies multiplier to result string
        Result = multiplier switch
        {
            0.0 => "No effect",
            < 1.0 => $"Not very effective ({multiplier}×)",
            > 1.0 => $"Super effective ({multiplier}×)",
            _ => "Normal effectiveness (1×)"
        };
    }
